...
/**
 * Firestore Trigger: Auto-feature park photos when likes reach threshold
 * - Collection: parks/{parkId}/photos/{photoId}
 * - When likes cross 10 and there are fewer than 3 auto-featured (non-pinned) photos,
...
exports.indexParkAliasesOnUpdate = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    try {
      const before = change.before.data() || {};
      const after = change.after.data() || {};
      // Cost guard: only re-index if relevant fields changed
...
exports.touchQueueTimestampOnParkUpdate = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    try {
      ...
      // Fast exit if counts differ (courts added/removed may include queue edits)
...
exports.pruneExpiredGoogleParks = functions.pubsub
  ...
  /**
   * Admin utility: Extend autoExpireAt for existing Google-sourced parks
   * Goal: move 30-day autoExpireAt to 90 days for already-imported docs
   *
   * Rules:
   * - Only touches parks where source=='places' (Google-derived)
   * - Computes target expiry as max(current autoExpireAt, createdAt + toDays)
   *   • If createdAt is missing or unparsable, extends by (toDays - assumeOldDays)
   * - Never reduces expiry; only extends forward when target > current
   * - Paginates by documentId to avoid composite index requirements
   * - Writes updatedAt alongside autoExpireAt
   */
  async function extendGoogleParksExpiryBatch(db, { toDays = 90, assumeOldDays = 30, pageSize = 1200, resume = true } = {}) {
    const ckptRef = db.collection('jobs').doc('extend_expiry_google');
    let cursorId = null;
    let totals = { pages: 0, scanned: 0, touched: 0, skipped: 0 };
    ...
    if (resume) {
      try {
        ...
        if (ck.exists) {
          ...
          totals.pages = Number(d.pages || 0);
          totals.scanned = Number(d.scanned || 0);
          totals.touched = Number(d.touched || 0);
          totals.skipped = Number(d.skipped || 0);
        }
      } catch (_) {}
    } else {
      try { await ckptRef.set({ status: 'in_progress', lastDocId: null, pages: 0, scanned: 0, touched: 0, skipped: 0, lastHeartbeatAt: new Date().toISOString() }, { merge: true }); } catch (_) {}
    }

    pageSize = Math.max(200, Math.min(1500, Number(pageSize) || 1200));
    let base = db.collection('parks')
      .where('source', '==', 'places')
    ...
    const snap = await base.limit(pageSize).get();
    if (snap.empty) {
      try { await ckptRef.set({ status: 'done', completedAt: new Date().toISOString() }, { merge: true }); } catch (_) {}
      return { pageScanned: 0, pageTouched: 0, pageSkipped: 0, done: true, cursor: null, ...totals };
    }
    ...
    let pageScanned = 0, pageTouched = 0, pageSkipped = 0;
    const docs = snap.docs;
    for (const doc of docs) {
      pageScanned += 1;
      const d = doc.data() || {};
      const currentExpIso = d.autoExpireAt || null;
      if (!currentExpIso) { pageSkipped += 1; continue; }
      let currentExp = new Date(currentExpIso);
      if (isNaN(currentExp.getTime())) { pageSkipped += 1; continue; }
      const createdIso = d.createdAt || null;
      let createdAt = createdIso ? new Date(createdIso) : null;
      if (createdAt && isNaN(createdAt.getTime())) createdAt = null;

      // Compute target expiry
      const addDays = (date, days) => { const x = new Date(date.getTime()); x.setUTCDate(x.getUTCDate() + Number(days)); return x; };
      let targetFromCreated = createdAt ? addDays(createdAt, Number(toDays)) : null;
      let delta = Math.max(0, Number(toDays) - Number(assumeOldDays || 30));
      let targetByDelta = addDays(currentExp, delta);
      let target = targetFromCreated && !isNaN(targetFromCreated.getTime()) ? targetFromCreated : targetByDelta;

      if (target <= currentExp) { pageSkipped += 1; continue; }

      const newIso = target.toISOString();
      batch.update(db.collection('parks').doc(doc.id), { autoExpireAt: newIso, updatedAt: new Date().toISOString() });
      writes += 1;
      pageTouched += 1;
      if (writes >= 400) await commit();
    }
    ...
    const newTotals = {
      pages: totals.pages + 1,
      scanned: totals.scanned + pageScanned,
      touched: totals.touched + pageTouched,
      skipped: totals.skipped + pageSkipped,
    };
    try {
      await ckptRef.set({
        status: isLast ? 'done' : 'in_progress',
    ...
    return { pageScanned, pageTouched, pageSkipped, done: isLast, cursor: isLast ? null : newCursor, ...newTotals };
  }
  ...
  exports.extendGoogleParkExpiryOnce = functions
    ...
    .https.onCall(async (data, context) => {
      ...
      const toDays = Math.max(1, Math.min(365, Number(data && data.toDays) || 90));
      const assumeOldDays = Math.max(0, Math.min(360, Number(data && data.assumeOldDays) || 30));
      const pageSize = Math.max(200, Math.min(1500, Number(data && data.pageSize) || 1200));
      const resume = (data && data.resume) !== false; // default true
      try {
        const res = await extendGoogleParksExpiryBatch(db, { toDays, assumeOldDays, pageSize, resume });
        return { ok: true, ...res, toDays, assumeOldDays };
      } catch (e) {
        throw new functions.https.HttpsError('internal', e?.message || 'Unknown error');
      }
    });
  ...
  exports.extendGoogleParkExpiryAll = functions
    ...
    .https.onCall(async (data, context) => {
      ...
      const pageSize = Math.max(200, Math.min(1500, Number(data && data.pageSize) || 1200));
      const t0 = Date.now();
      let agg = { pages: 0, scanned: 0, touched: 0, skipped: 0 };
      let done = false;
      do {
        const out = await extendGoogleParksExpiryBatch(db, { toDays, assumeOldDays, pageSize, resume: true });
        agg.pages = out.pages;
        agg.scanned = out.scanned;
        agg.touched = out.touched;
        agg.skipped = out.skipped;
        done = !!out.done;
        if (!done && (Date.now() - t0) > 520000) break; // leave headroom
      } while (!done);
      return { ok: true, ...agg, done, toDays, assumeOldDays };
    });
  ...
  exports.extendGoogleParkExpiryAllHttp = functions
    ...
    .https.onRequest(async (req, res) => {
      try {
        ...
        if (!secretConfigured || !provided || provided !== secretConfigured) { res.status(401).json({ ok: false, error: 'Unauthorized' }); return; }
        const db = admin.firestore();
        const toDays = Math.max(1, Math.min(365, Number((req.body && req.body.toDays) || 90)));
        const assumeOldDays = Math.max(0, Math.min(360, Number((req.body && req.body.assumeOldDays) || 30)));
        const pageSize = Math.max(200, Math.min(1500, Number((req.body && req.body.pageSize) || 1200)));
        const t0 = Date.now();
        let agg = { pages: 0, scanned: 0, touched: 0, skipped: 0 };
        let done = false;
        ...
        res.status(200).json({ ok: true, ...agg, done, toDays, assumeOldDays });
      } catch (e) {
        res.status(500).json({ ok: false, error: e?.message || 'Unhandled error' });
      }
    });
...
// Removed per owner request: runParksBackfillOnce (callable)
 ...
 */
// Removed per owner request: runParksBackfillOnceHttp (HTTP)

// Optional: unattended scheduler that consumes batches using backfill/control
// Removed per owner request: scheduledParksBackfillBatch (Pub/Sub)
...
async function sendNotificationsToUsers(db, userIds, title, body, data) {
  ...
  // Send in chunks of 500 to respect FCM limits
  const chunkSize = 500;
  let totalSuccess = 0;
  let totalFailure = 0;
      ...
      resp.responses.forEach((r, idx) => {
        if (!r.success) {
          ...
          if (errorCode === 'messaging/invalid-registration-token' ||
            ...
            allInvalidTokens.push(slice[idx]);
          }
        }
      });
    }
  ...
  console.log(`Successfully sent ${totalSuccess} notification(s); failures=${totalFailure}`);

  // Clean up invalid tokens across all chunks
  if (allInvalidTokens.length > 0) {
    const deletePromises = allInvalidTokens.map(token => {
    ...
    console.log(`Cleaned up ${allInvalidTokens.length} invalid token(s)`);
  }
 ...
 */
exports.onImageFinalize = onObjectFinalized({ bucket: 'courthub-app.firebasestorage.app', region: REGION, timeoutSeconds: 120, memory: '1GiB' },
  async (event) => {
    try {
